...
/**
 * Main method to start a pre-loaded server (see SERVER_TYPES). 
 * 
 * This can connect to an RMI registry or create one if needed
...
public class StartRegistryAndServer {
	...
	public static void main(String args[]) throws IOException, ClassNotFoundException, AlreadyBoundException, InstantiationException, IllegalAccessException{
		...
		if (cmd.hasOption("n")) {
			...
			// set systems properties for hostname??
			if(cmd.hasOption("sp")){
				setProperty = true;
			}
		}
		...
		// set java property needed on system
		if(setProperty){
			System.setProperty("java.rmi.server.hostname", hostname);
			System.err.println("Set property java.rmi.server.hostname to "+hostname);
		}

		// either connect to existing registry
		// ... or start a new registry
		Registry registry;
		if(startReg){
		...
		// create a remote skeleton to make it 
		// ready for incoming calls
		//
		// important to keep the reference to skeleton!
		
		server = serverClass.newInstance();
		System.err.println("Creating server for "+server.getClass());
		
		Remote skel = UnicastRemoteObject.exportObject(server,0);
		String skelname = serverClass.getSimpleName();
		
		// 	add the skeleton to the registry
		registry.bind(skelname, skel);
